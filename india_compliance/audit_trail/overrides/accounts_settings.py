...
def validate_change_in_enable_audit_trail(doc):
    ...
    if not doc.enable_audit_trail:
        dontmanage.throw(_("Audit Trail cannot be disabled once enabled"))

    # Enable audit trail
    doc.delete_linked_ledger_entries = 0
    dontmanage.enqueue(create_property_setters_for_versioning, queue="short", at_front=True)
    ...
    if doc.enable_audit_trail and doc.delete_linked_ledger_entries:
        dontmanage.throw(
            _("{0} cannot be enabled to ensure Audit Trail integrity").format(
                dontmanage.bold(doc.meta.get_label("delete_linked_ledger_entries"))
            )
        )
