...
def update_pan_for_company():
    ...
    if not dontmanage.db.has_column("Company", "pan_details"):
        return

    company = dontmanage.qb.DocType("Company")
    dontmanage.qb.update(company).set(company.pan, company.pan_details).run()


def update_na_gstin():
    for doctype in ("Address", "Customer", "Supplier"):
        dontmanage.db.set_value(doctype, {"gstin": "NA"}, "gstin", "")
...
def update_gstin_and_gst_category():
    ...
    all_addresses = dontmanage.get_all(
        "Address",
        fields=(
            "name",
            "gstin",
    ...
    for doctype in GST_PARTY_TYPES:
        for party in dontmanage.get_all(doctype, fields=("name", "gstin", "gst_category")):
    ...
    for (doctype, gstin), docnames in new_gstins.items():
        dontmanage.db.set_value(doctype, {"name": ("in", docnames)}, "gstin", gstin)
