...
def migrate_e_waybill_fields():
    if not dontmanage.db.has_column("Sales Invoice", "eway_bill_validity"):
        return
...
def migrate_e_invoice_fields():
    if not dontmanage.db.has_column("Sales Invoice", "irn_cancelled"):
        return
...
def migrate_e_invoice_request_log():
    if not dontmanage.db.table_exists("E Invoice Request Log"):
        return
    ...
    # since the DocType E Invoice Request Log gets deleted in DontManageErp patch
    docs = dontmanage.db.get_values(
