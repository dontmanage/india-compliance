...
from dontmanage.query_builder import DatePart
from dontmanage.query_builder.functions import Extract, Sum
from dontmanage.utils import cstr, flt, get_date_str, get_first_day, get_last_day
...
class GSTR3BReport(Document):
    ...
    def get_data(self):
        ...
        self.json_output = dontmanage.as_json(self.report_dict)
    ...
    def get_itc_reversal_entries(self):
        reversal_entries = dontmanage.db.sql(
            """
    ...
    def update_imports_from_bill_of_entry(self, itc_details):
        boe = dontmanage.qb.DocType("Bill of Entry")
        boe_taxes = dontmanage.qb.DocType("Bill of Entry Taxes")
        gst_accounts = get_gst_accounts_by_type(self.company, "Input")

        def _get_tax_amount(account_type):
            return (
                dontmanage.qb.from_(boe)
    ...
    def get_inward_nil_exempt(self, state):
        inward_nil_exempt = dontmanage.db.sql(
            """
    ...
    def get_outward_tax_details(self, doctype):
        ...
        self.invoice_cess = dontmanage._dict()
    ...
    def get_company_gst_details(self):
        gst_details = dontmanage.get_all(
            "Address",
    ...
    def get_account_heads(self):
        ...
        account_heads = {}
        gst_settings_accounts = dontmanage.get_all(
            "GST Account",
            filters={
        ...
        if not gst_settings_accounts:
            dontmanage.throw(_("Please set GST Accounts in GST Settings"))
...
def get_address_state_map():
    return dontmanage._dict(
        dontmanage.get_all("Address", fields=["name", "gst_state"], as_list=1)
    )
...
def view_report(name):
    dontmanage.has_permission("GSTR 3B Report", throw=True)

    json_data = dontmanage.get_value("GSTR 3B Report", name, "json_output")
...
def make_json(name):
    ...
    dontmanage.local.response.filename = file_name
    dontmanage.local.response.filecontent = json_data
