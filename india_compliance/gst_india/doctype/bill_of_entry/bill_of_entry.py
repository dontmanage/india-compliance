...
class BillofEntry(Document):
    ...
    def set_item_defaults(self):
        ...
        for item in self.items:
            item.name = dontmanage.generate_hash(length=10)
            item.customs_duty = 0

    def set_default_accounts(self):
        company = dontmanage.get_cached_doc("Company", self.company)
    ...
    def validate_purchase_invoice(self):
        purchase = dontmanage.get_doc("Purchase Invoice", self.purchase_invoice)
            ...
            if item.pi_detail not in pi_items:
                dontmanage.throw(
                    _(
                        ...
                        item.idx,
                        dontmanage.bold(item.pi_detail),
                        dontmanage.bold(self.purchase_invoice),
                    )
                )
    ...
    def get_gl_entries(self):
        ...
        self.company_currency = dontmanageerp.get_company_currency(self.company)
    ...
    def validate_account_currency(self, account, account_currency=None):
        ...
        dontmanage.throw(
            _("Row #{0}: Account {1} must be of INR currency").format(
                self.idx, dontmanage.bold(account)
            )
        )
...
def make_bill_of_entry(source_name, target_doc=None):
    def set_missing_values(source, target):
        ...
        rate, description = dontmanage.db.get_value(
...
def make_journal_entry_for_payment(source_name, target_doc=None):
    def set_missing_values(source, target):
        ...
        company = dontmanage.get_cached_doc("Company", source.company)
        target.append(
            "accounts",
            {
...
def make_landed_cost_voucher(source_name, target_doc=None):
    def set_missing_values(source, target):
        ...
        if not items:
            dontmanage.throw(_("No items found for Landed Cost Voucher"))
...
def get_items_for_landed_cost_voucher(boe):
    ...
    """
    pi = dontmanage.get_doc("Purchase Invoice", boe.purchase_invoice)
    item_customs_map = {item.pi_detail: item.customs_duty for item in boe.items}

    def _item_dict(items):
        return dontmanage._dict({item.name: item for item in items})
