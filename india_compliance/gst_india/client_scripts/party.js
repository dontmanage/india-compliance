...
function update_gstin_in_other_documents(doctype) {
    dontmanage.ui.form.on(doctype, {
        after_save(frm) {
            ...
            const { docs_with_previous_gstin, previous_gstin } = dontmanage.last_response;
            ...
            for (const [doctype, docnames] of Object.entries(
                ...
                docnames.forEach(docname => {
                    message += `${dontmanage.utils.get_form_link(
                        doctype,
                        docname,
                        true
                    )}<br>`;
            ...
            dontmanage.confirm(message, function () {
                dontmanage.call({
...
function validate_gstin(doctype) {
    dontmanage.ui.form.on(doctype, {
        gstin(frm) {
            ...
            // TODO: remove below condition once event is fixed in dontmanage
            if (!gstin || gstin.length < 15) return;

            if (gstin.length > 15) {
                dontmanage.throw(__("GSTIN/UIN should be 15 characters long"));
            }
...
function validate_pan(doctype) {
    dontmanage.ui.form.on(doctype, {
        pan(frm) {
            ...
            if (pan.length > 10) {
                dontmanage.throw(__("PAN should be 10 characters long"));
            }

            pan = pan.trim().toUpperCase();

            if (!PAN_REGEX.test(pan)) {
                dontmanage.throw(__("Invalid PAN format"));
            }
