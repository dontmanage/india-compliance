function setup_e_waybill_actions(doctype) {
    ...
    dontmanage.ui.form.on(doctype, {
        ...
        setup(frm) {
            ...
            dontmanage.realtime.on("e_waybill_pdf_update", message => {
                dontmanage.model.sync_docinfo(message);
        ...
        refresh(frm) {
            ...
            if (!frm.doc.ewaybill) {
                if (dontmanage.perm.has_perm(frm.doctype, 0, "submit", frm.doc.name)) {
            ...
            if (dontmanage.model.can_print("e-Waybill Log")) {
                frm.add_custom_button(
                    __("Print"),
                    () => {
                        dontmanage.set_route("print", "e-Waybill Log", frm.doc.ewaybill);
                    },
                    "e-Waybill"
                );
            }

            if (dontmanage.perm.has_perm(frm.doctype, 0, "write", frm.doc.name)) {
                frm.add_custom_button(
                    __("Attach"),
        ...
        async on_submit(frm) {
            ...
            dontmanage.show_alert(__("Attempting to generate e-Waybill"));
...
function show_generate_e_waybill_dialog(frm) {
    ...
    const json_action = async values => {
        const ewb_data = await dontmanage.xcall(
    ...
    if (
        ...
        $(`
            <div class="alert alert-warning" role="alert">
                ...
                <a
                    href="https://docs.dontmanageerp.com/docs/v14/user/manual/en/regional/india/generating_e_invoice#what-if-we-generate-e-waybill-before-the-e-invoice"
                    class="alert-link"
                    target="_blank"
                >
                    Learn more
...
async function update_gst_tranporter_id(dialog) {
    ...
    const { message: response } = await dontmanage.db.get_value(
        "Supplier",
        transporter,
        "gst_transporter_id"
    );
...
function get_e_waybill_file_name(docname) {
    ...
    return `${prefix}_e-Waybill_Data_${dontmanage.utils.get_random(5)}.json`;
}
