...
const dontmanageerp_onload = dontmanage.listview_settings[DOCTYPE].onload;
dontmanage.listview_settings[DOCTYPE].onload = function (list_view) {
    if (dontmanageerp_onload) {
        dontmanageerp_onload(list_view);
    }

    if (!dontmanage.perm.has_perm(DOCTYPE, 0, "submit")) return;
...
async function enqueue_bulk_e_invoice_generation(docnames) {
    const now = dontmanage.datetime.system_datetime();

    const job_id = await dontmanage.xcall(
        "india_compliance.gst_india.utils.e_invoice.enqueue_bulk_e_invoice_generation",
        { docnames }
    );

    const creation_filter = `[">", "${now}"]`;
    const api_requests_link = dontmanage.utils.generate_route({
        type: "doctype",
    ...
    const error_logs_link = dontmanage.utils.generate_route({
        type: "doctype",
        name: "Error Log",
        route_options: {
            creation: creation_filter,
    ...
    dontmanage.msgprint(
        __(
            ...
            [
                dontmanage.utils.get_form_link("RQ Job", job_id),
                api_requests_link,
                error_logs_link,
            ]
        )
...
async function validate_if_submitted(selected_docs) {
    ...
    if (!valid_docs.length) {
        dontmanage.throw(__("This action can only be performed on submitted documents"));
    }
