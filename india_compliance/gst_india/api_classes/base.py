...
class BaseAPI:
    ...
    def __init__(self, *args, **kwargs):
        self.settings = dontmanage.get_cached_doc("GST Settings")
        ...
        self.default_headers = {
            "x-api-key": (
                ...
                or dontmanage.conf.ic_api_secret
            )
        }
    ...
    def _make_request(
        ...
        if method not in ("GET", "POST"):
            dontmanage.throw(_("Invalid method {0}").format(method))

        request_args = dontmanage._dict(
        ...
        try:
            ...
            try:
                response_json = response.json(object_hook=dontmanage._dict)
            except Exception:
                pass
            ...
            if not response_json:
                dontmanage.throw(_("Error parsing response: {0}").format(response.content))
            else:
            ...
            if not success_value and not self.handle_failed_response(response_json):
                dontmanage.throw(
                    ...
                    or dontmanage.as_json(response_json, indent=4),
            ...
            if self.sandbox_mode and not dontmanage.flags.ic_sandbox_message_shown:
                dontmanage.msgprint(
                    _("GST API request was made in Sandbox Mode"),
                    alert=True,
                )
                dontmanage.flags.ic_sandbox_message_shown = True

    def handle_failed_response(self, response_json):
        # Override in subclass, return truthy value to stop dontmanage.throw
        pass
    ...
    def generate_request_id(self, length=12):
        return dontmanage.generate_hash(length=length)
