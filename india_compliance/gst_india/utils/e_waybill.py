...
from dontmanage.desk.form.load import get_docinfo
from dontmanage.utils import add_to_date, get_datetime, get_fullname, random_string
...
def generate_e_waybill_json(doctype: str, docnames, values=None):
    docnames = dontmanage.parse_json(docnames) if docnames.startswith("[") else [docnames]
    ewb_data = {
        "version": "1.0.0621",
        "billLists": [],
    }
    ...
    for doc in docnames:
        doc = dontmanage.get_doc(doctype, doc)
        doc.check_permission("submit")

        if values:
            update_transaction(doc, dontmanage.parse_json(values))
            send_updated_doc(doc)

        ewb_data["billLists"].append(EWaybillData(doc, for_json=True).get_data())

    return dontmanage.as_json(ewb_data, indent=4)
...
def log_and_process_e_waybill_generation(doc, result, *, with_irn=False):
    ...
    log_and_process_e_waybill(
        ...
        fetch=dontmanage.get_cached_value(
...
def update_vehicle_info(*, doctype, docname, values):
    ...
    ).format(user=dontmanage.bold(get_fullname()))
    ...
    for key, value in values_in_comment.items():
        if value:
            comment += "{0}: {1} <br>".format(dontmanage.bold(_(key)), value)
...
def update_transporter(*, doctype, docname, values):
    ...
    transporter_name = (
        dontmanage.db.get_value("Supplier", values.transporter, "supplier_name")
    ...
    comment = (
        ...
        user=dontmanage.bold(get_fullname()),
        transporter_id=dontmanage.bold(values.gst_transporter_id),
    )
...
def fetch_e_waybill_data(*, doctype, docname, attach=False):
    ...
    log = dontmanage.get_doc("e-Waybill Log", doc.ewaybill)
...
def _fetch_e_waybill_data(doc, log):
    ...
    log.db_set(
        {
            "data": dontmanage.as_json(result, indent=4),
            "is_latest_data": 1,
        }
    )
...
def attach_e_waybill_pdf(doc, log=None):
    pdf_content = dontmanage.get_print(
        "e-Waybill Log",
        doc.ewaybill,
        "e-Waybill",
        doc=log,
...
def delete_file(doc, filename):
    ...
    for file in dontmanage.get_all(
        "File",
        filters=[
        ...
        dontmanage.delete_doc("File", file, force=True, ignore_permissions=True)


def publish_pdf_update(doc, pdf_deleted=False):
    get_docinfo(doc)

    # if it's a request, dontmanage.response["docinfo"] will get synced automatically
...
def _log_and_process_e_waybill(doc, log_data, fetch=False, comment=None):
    ...
    try:
        log = dontmanage.get_doc("e-Waybill Log", log_name)
    except dontmanage.DoesNotExistError:
        log = dontmanage.new_doc("e-Waybill Log")
        dontmanage.clear_last_message()
...
class EWaybillData(GSTTransactionData):
        ...
        if self.doc.ewaybill:
            dontmanage.throw(
                _("e-Waybill already generated for {0} {1}").format(
                    _(self.doc.doctype), dontmanage.bold(self.doc.name)
                )
            )
    ...
    def validate_settings(self):
        if not self.settings.enable_e_waybill:
            dontmanage.throw(_("Please enable e-Waybill in GST Settings"))
    ...
    def validate_if_e_waybill_is_set(self):
        if not self.doc.ewaybill:
            dontmanage.throw(_("No e-Waybill found for this document"))
    ...
    def check_e_waybill_validity(self):
        ...
        if valid_upto and get_datetime(valid_upto) < get_datetime():
            dontmanage.throw(_("e-Waybill cannot be modified after its validity is over"))
