...
def enqueue_bulk_e_invoice_generation(docnames):
    ...
    dontmanage.has_permission("Sales Invoice", "submit", throw=True)

    gst_settings = dontmanage.get_cached_doc("GST Settings")
    if not is_api_enabled(gst_settings) or not gst_settings.enable_e_invoice:
        dontmanage.throw(_("Please enable e-Invoicing in GST Settings first"))

    docnames = dontmanage.parse_json(docnames) if docnames.startswith("[") else [docnames]
    rq_job = dontmanage.enqueue(
...
def generate_e_invoice(docname, throw=True):
    ...
    if result.SignedInvoice:
        ...
        invoice_data = dontmanage.as_json(decoded_invoice, indent=4)
...
def _log_e_invoice(log_data):
    ...
    try:
        log = dontmanage.get_doc("e-Invoice Log", log_name)
    except dontmanage.DoesNotExistError:
        log = dontmanage.new_doc("e-Invoice Log")
        dontmanage.clear_last_message()
    ...
    if getdate(gst_settings.e_invoice_applicable_from) > getdate(doc.posting_date):
        return _throw(
            _(
                ...
                " GST Settings"
            ).format(dontmanage.bold(format_date(gst_settings.e_invoice_applicable_from)))
        )
...
def validate_if_e_invoice_can_be_cancelled(doc):
    if not doc.irn:
        dontmanage.throw(_("IRN not found"), title=_("Error Cancelling e-Invoice"))
...
class EInvoiceData(GSTTransactionData):
    ...
    def update_item_details(self, item_details, item):
        ...
        if batch_no := self.sanitize_value(
            ...
            batch_expiry_date = dontmanage.db.get_value(
