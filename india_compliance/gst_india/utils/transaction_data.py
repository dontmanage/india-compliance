...
from dontmanage.utils import format_date, get_link_to_form, getdate, rounded
...
class GSTTransactionData:
    ...
    def __init__(self, doc):
        ...
        self.transaction_details = dontmanage._dict()
    ...
    def validate_mode_of_transport(self, throw=True):
        def _throw(error):
            if throw:
                dontmanage.throw(error, title=_("Invalid Transporter Details"))
    ...
    def update_item_tax_details(self, item_details, item):
        ...
        for row in self.doc.taxes:
            ...
            tax_rate = self.rounded(
                dontmanage.parse_json(row.item_wise_tax_detail).get(
    ...
    def get_address_details(self, address_name, validate_gstin=False):
        address = dontmanage.get_cached_value(
            "Address",
            address_name,
            (
                "name",
        ...
        for fieldname in fieldnames:
            ...
            dontmanage.throw(
                _(
                    ...
                    dontmanage.bold(dontmanage.get_meta("Address").get_label(fieldname)),
                    dontmanage.bold(address.name),
                ),
        ...
        if not PINCODE_FORMAT.match(address.pincode):
            dontmanage.throw(
                _(
                    ...
                    " with 0"
                ).format(dontmanage.bold(address.name)),
                title=_("Invalid Data"),
            )
        ...
        *,
        ...
        def _throw(message, **format_args):
            ...
            message = message.format(
                field=_(dontmanage.get_meta(reference_doctype).get_label(fieldname)),
                **format_args,
            )
            ...
            dontmanage.throw(
                _("{reference_doctype} {reference_link}: {message}").format(
                    ...
                    reference_link=dontmanage.bold(
