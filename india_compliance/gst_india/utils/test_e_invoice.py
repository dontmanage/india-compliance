...
from dontmanage.utils import add_to_date, get_datetime, getdate, now_datetime
...
class TestEInvoice(DontManageTestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.e_invoice_test_data = dontmanage._dict(
            dontmanage.get_file_json(
                dontmanage.get_app_path(
    ...
    def test_generate_e_invoice_with_goods_item(self):
        ...
        self.assertDocumentEqual(
            {
                "output": dontmanage.as_json(test_data.get("response_data"), indent=4),
            },
            dontmanage.get_doc(
                "Integration Request",
        ...
        self.assertDocumentEqual(
            ...
            dontmanage.get_doc("Sales Invoice", si.name),
        )

        self.assertDocumentEqual(
            {"name": test_data.get("response_data").get("result").get("Irn")},
            dontmanage.get_doc("e-Invoice Log", {"sales_invoice": si.name}),
        )
        self.assertDocumentEqual(
            {"name": test_data.get("response_data").get("result").get("EwbNo")},
            dontmanage.get_doc("e-Waybill Log", {"reference_name": si.name}),
        )
    ...
    def test_generate_e_invoice_with_service_item(self):
        ...
        self.assertFalse(
            dontmanage.db.get_value("e-Waybill Log", {"reference_name": si.name}, "name")
        )
    ...
    def test_return_e_invoice_with_goods_item(self):
        ...
        self.assertDictEqual(
            ...
            EInvoiceData(dontmanage.get_doc("Sales Invoice", return_si.name)).get_data(),
        )
        ...
        self.assertDocumentEqual(
            ...
            dontmanage.get_doc("Sales Invoice", return_si.name),
        )

        self.assertDocumentEqual(
            {"name": test_data.get("response_data").get("result").get("Irn")},
            dontmanage.get_doc("e-Invoice Log", {"sales_invoice": return_si.name}),
        )
    ...
    def test_debit_note_e_invoice_with_goods_item(self):
        ...
        self.assertDocumentEqual(
            ...
            dontmanage.get_doc("Sales Invoice", debit_note.name),
        )

        self.assertDocumentEqual(
            {"name": test_data.get("response_data").get("result").get("Irn")},
            dontmanage.get_doc("e-Invoice Log", {"sales_invoice": debit_note.name}),
        )
    ...
    def _cancel_e_invoice(self, invoice_no):
        ...
        return dontmanage.get_doc("Sales Invoice", doc.name)
...
def update_dates_for_test_data(test_data):
    ...
    today = format_date(dontmanage.utils.today(), "dd/mm/yyyy")
