...
from dontmanage.utils import flt, formatdate, getdate
        ...
        for item in report_data[:-1]:
            if not item.get("place_of_supply"):
                dontmanage.throw(
                    ...
                    ).format(dontmanage.bold("Place Of Supply"))
                )
            ...
            if not invoice[0]["place_of_supply"]:
                dontmanage.throw(
					...
					Please update and try again"""
                        ...
                        dontmanage.bold("Place Of Supply"),
                        dontmanage.bold(invoice[0]["invoice_number"]),
                    )
                )
...
def get_company_gstin_number(company, address=None, all_gstins=False):
    gstin = ""
    if address:
        gstin = dontmanage.db.get_value("Address", address, "gstin")
    ...
    if not gstin:
        address = dontmanage.bold(address) if address else ""
        dontmanage.throw(
            _("Please set valid GSTIN No. in Company Address {} for company {}").format(
                address, dontmanage.bold(company)
            )
        )
...
def download_json_file():
    ...
    dontmanage.response["filename"] = (
        dontmanage.scrub("{0} {1}".format(data["report_name"], data["report_type"]))
        + ".json"
    )
