...
class TestHSNWiseSummaryReport(TestCase):
    ...
    def test_hsn_summary_for_invoice_with_duplicate_items(self):
        ...
        append_item(si_one, dontmanage._dict(gst_hsn_code="61149090", uom="Box"))
        append_item(si_one, dontmanage._dict(gst_hsn_code="61149090", uom="Litre"))
        si_one.submit()

        si_two = create_sales_invoice(do_not_save=1, is_in_state=True)
        append_item(si_two, dontmanage._dict(gst_hsn_code="61149090", uom="Box"))
        append_item(si_two, dontmanage._dict(gst_hsn_code="61149090", uom="Litre"))
