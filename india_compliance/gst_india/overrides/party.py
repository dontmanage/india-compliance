...
def validate_pan(doc):
    ...
    if not is_valid_pan(doc.pan):
        dontmanage.throw(_("Invalid PAN format"))


def set_docs_with_previous_gstin(doc, method=None):
    if not dontmanage.request or dontmanage.flags.in_update_docs_with_previous_gstin:
        return
    ...
    dontmanage.response.docs_with_previous_gstin = docs_with_previous_gstin
    dontmanage.response.previous_gstin = previous_gstin
...
def get_docs_with_previous_gstin(gstin, doctype, docname):
    ...
    for dt in ("Address", "Supplier", "Customer", "Company"):
        for doc in dontmanage.get_list(dt, filters={"gstin": gstin}):
...
def update_docs_with_previous_gstin(gstin, gst_category, docs_with_previous_gstin):
    dontmanage.flags.in_update_docs_with_previous_gstin = True
    ...
    dontmanage.msgprint(_("GSTIN Updated"), indicator="green", alert=True)


def create_primary_address(doc, method=None):
    """
    Used to create primary address when creating party.
    Modified version of dontmanageerp.selling.doctype.customer.customer.create_primary_address

    DontManageErp uses `address_line1` so we use `_address_line1` to avoid conflict.
    """
