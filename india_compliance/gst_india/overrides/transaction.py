...
from dontmanage.model import delete_doc
...
def is_indian_registered_company(doc):
    if not doc.company_gstin:
        country, gst_category = dontmanage.get_cached_value(
...
def validate_mandatory_fields(doc, fields, message=None):
    ...
    for field in fields:
        if not doc.get(field):
            ...
            dontmanage.throw(error_message, title=_("Missing Required Field"))
...
def validate_gst_accounts(doc, is_sales_transaction=False):
    ...
    def _throw(message, title=None):
        dontmanage.throw(message, title=title or _("Invalid GST Account"))
...
def validate_items(doc):
    ...
    item_tax_templates = dontmanage._dict()
...
def validate_hsn_codes(doc, method=None):
    ...
    if doc._action == "submit":
        ...
        dontmanage.throw(
            _(
                ...
                " <br>{0}"
            ).format(dontmanage.bold(", ".join(rows_with_invalid_hsn)))
        )
    ...
    if rows_with_missing_hsn:
        dontmanage.msgprint(
            ...
            ).format(dontmanage.bold(", ".join(rows_with_missing_hsn)))
        )
    ...
    if rows_with_invalid_hsn:
        dontmanage.msgprint(
            _(
                ...
                " row numbers: <br>{1}"
            ).format(min_hsn_digits, dontmanage.bold(", ".join(rows_with_invalid_hsn)))
        )
...
def validate_overseas_gst_category(doc, method=None):
    ...
    overseas_enabled = dontmanage.get_cached_value(
    ...
    if not overseas_enabled:
        dontmanage.throw(
            ...
            ).format(dontmanage.bold(doc.gst_category))
        )

    if doc.doctype == "POS Invoice":
        dontmanage.throw(_("Cannot set GST Category to SEZ / Overseas in POS Invoice"))
...
def get_itemised_tax_breakup_header(item_doctype, tax_accounts):
    ...
    if (
        dontmanage.get_meta(item_doctype).has_field("gst_hsn_code")
...
def get_regional_round_off_accounts(company, account_list):
    country = dontmanage.get_cached_value("Company", company, "country")
    ...
    if not dontmanage.db.get_single_value("GST Settings", "round_off_gst_values"):
        return
...
def get_gst_details(party_details, doctype, company, *, update_place_of_supply=False):
    ...
    if isinstance(party_details, str):
        party_details = dontmanage.parse_json(party_details)

    gst_details = dontmanage._dict()
...
def get_tax_template_based_on_category(master_doctype, company, party_details):
    ...
    default_tax = dontmanage.db.get_value(
        master_doctype,
...
def get_tax_template(master_doctype, company, is_inter_state, state_code):
    tax_categories = dontmanage.get_all(
        "Tax Category",
...
def set_item_tax_from_hsn_code(item):
    if not item.taxes and item.gst_hsn_code:
        hsn_doc = dontmanage.get_doc("GST HSN Code", item.gst_hsn_code)
...
def validate_reverse_charge_transaction(doc, method=None):
    ...
    if base_gst_tax != base_reverse_charge_booked:
        ...
        msg += _(
            ...
            gst_document_link=(
                '<a href="https://docs.dontmanageerp.com/docs/user/manual/en/regional/india/gst-setup">GST'
                " Documentation</a>"
            )
        )
...
# Note: This is kept for backwards compatibility with DontManage versions < 14.21.0
