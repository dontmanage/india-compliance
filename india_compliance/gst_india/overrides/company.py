...
from dontmanageerp.setup.setup_wizard.operations.taxes_setup import from_detailed_data

from india_compliance.gst_india.utils import get_data_file_path


def delete_gst_settings_for_company(doc, method=None):
    if not dontmanage.flags.country_change or doc.country != "India":
        return

    gst_settings = dontmanage.get_doc("GST Settings")
...
def make_default_tax_templates(company: str):
    if not dontmanage.db.exists("Company", company):
        dontmanage.throw(
            _("Company {0} does not exist yet. Taxes setup aborted.").format(company)
        )

    dontmanage.has_permission("Company", ptype="write", doc=company, throw=True)

    default_taxes = dontmanage.get_file_json(get_data_file_path("tax_defaults.json"))
...
def update_gst_settings(company):
    ...
    gst_settings = dontmanage.get_single("GST Settings")
