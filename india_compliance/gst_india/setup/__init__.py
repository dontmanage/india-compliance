...
def create_accounting_dimension_fields():
    doctypes = dontmanage.get_hooks(
        "accounting_dimension_doctypes",
        app_name="india_compliance",
    )

    dimensions = dontmanage.get_all("Accounting Dimension", pluck="name")
    for dimension in dimensions:
        doc = dontmanage.get_doc("Accounting Dimension", dimension)
        make_dimension_in_accounting_doctypes(doc, doctypes)


def create_property_setters():
    for property_setter in get_property_setters():
        dontmanage.make_property_setter(property_setter)


def create_address_template():
    if dontmanage.db.exists("Address Template", "India"):
        return

    address_html = dontmanage.read_file(
...
def create_hsn_codes():
    ...
    hsn_codes = [
        ...
        for code in dontmanage.get_file_json(get_data_file_path("hsn_codes.json"))
    ]
...
def set_default_gst_settings():
    settings = dontmanage.get_doc("GST Settings")
    default_settings = {
...
def set_default_accounts_settings():
    ...
    dontmanage.db.set_default("add_taxes_from_item_tax_template", 0)
...
def show_accounts_settings_override_warning():
    ...
    address_for_tax_category = dontmanage.db.get_value(
        "Accounts Settings",
        "Accounts Settings",
