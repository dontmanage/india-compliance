...
def run_post_install_patches():
    ...
    try:
        for patch in POST_INSTALL_PATCHES:
            dontmanage.get_attr(f"india_compliance.patches.post_install.{patch}.execute")()
...
def disable_ic_account_page():
    """
    Disable the India Compliance Account Page if API secret is set in dontmanage.conf
    """

    if not dontmanage.conf.ic_api_secret or dontmanage.db.exists(
        "Custom Role", {"page": "india-compliance-account"}
    ):
        return

    dontmanage.get_doc(doctype="Custom Role", page="india-compliance-account").insert()
