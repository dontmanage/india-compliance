...
dontmanage.provide("india_compliance");

window.gst_settings = dontmanage.boot.gst_settings;
...
Object.assign(india_compliance, {
    ...
    async get_gstin_options(party, party_type = "Company") {
        ...
        const { message } = await dontmanage.call({
            method: query,
            args: params,
        });
        return message;
    ...
    get_party_type(doctype) {
        return in_list(dontmanage.boot.sales_doctypes, doctype) ? "Customer" : "Supplier";
    },
    ...
    set_state_options(frm) {
        ...
        state_field.set_data(dontmanage.boot.india_state_options || []);
    },

    can_enable_api(settings) {
        return settings.api_secret || dontmanage.boot.ic_api_enabled_from_conf;
    },
