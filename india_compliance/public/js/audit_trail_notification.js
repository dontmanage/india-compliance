$(document).on("app_ready", async function() {
    if (!dontmanage.boot.needs_audit_trail_notification) return;

    // let other processes finish
    await new Promise(resolve => setTimeout(resolve, 700));
    const d = dontmanage.msgprint({
    ...
    d.set_secondary_action(() => {
        dontmanage.set_route("Form", "Accounts Settings");
        d.hide();
    });

    d.onhide = () => {
        dontmanage.xcall("india_compliance.audit_trail.utils.disable_audit_trail_notification");
    };
});
