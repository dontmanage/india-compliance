...
class GSTQuickEntryForm extends dontmanage.ui.form.QuickEntryForm {
    constructor(...args) {
        super(...args);
    ...
    get_address_fields() {
        return [
            ...
            {
                label: __("Pincode"),
                // set as _pincode so that dontmanage.ui.form.Layout doesn't override it
                fieldname: "_pincode",
            ...
            },
            {
                ...
                fieldtype: "Link",
                options: "Country",
                default: dontmanage.defaults.get_user_default("country"),
                onchange: () => {
                    india_compliance.set_state_options(this.dialog);
                },
            },
...
class PartyQuickEntryForm extends GSTQuickEntryForm {
    ...
    update_doc() {
        const doc = super.update_doc();
        // to prevent clash with DontManageErp
...
dontmanage.ui.form.CustomerQuickEntryForm = PartyQuickEntryForm;
dontmanage.ui.form.SupplierQuickEntryForm = PartyQuickEntryForm;
...
class AddressQuickEntryForm extends GSTQuickEntryForm {
        ...
        return [
            ...
            },
            {
                ...
                onchange: async () => {
                    ...
                    if (
                        !link_name ||
                        !in_list(dontmanage.boot.gst_party_types, link_doctype)
                    )
                        return;

                    const { message: gstin_list } = await dontmanage.call(
    ...
    update_doc() {
        ...
        if (doc.link_doctype && doc.link_name) {
            const link = dontmanage.model.add_child(doc, "Dynamic Link", "links");
    ...
    get_default_party() {
        ...
        if (
            ...
            dontmanage.dynamic_link.doc === doc
        ) {
            return {
                party_type: dontmanage.dynamic_link.doctype,
                party: dontmanage.dynamic_link.doc[dontmanage.dynamic_link.fieldname]
            };
        }
    }
}

dontmanage.ui.form.AddressQuickEntryForm = AddressQuickEntryForm;
...
function update_party_info(doc, gstin_info) {
    ...
    if (!in_list(dontmanage.boot.gst_party_types, doc.doctype)) return;
...
function update_address_info(doc, address) {
    ...
    // set field renamed due conflict with dontmanage.ui.form.Layout
