...
def create_company_fixtures(company):
    company = company or dontmanage.db.get_value("Global Defaults", None, "default_company")
...
def set_tax_withholding_category(company):
    ...
    abbr = dontmanage.get_value("Company", company, "abbr")
    tds_account = dontmanage.get_value("Account", "TDS Payable - {0}".format(abbr), "name")
    ...
    for d in docs:
        if not dontmanage.db.exists("Tax Withholding Category", d.get("name")):
...
def get_tds_details(accounts, fiscal_year_details):
    tds_details = []
    tds_rules = dontmanage.get_file_json(
